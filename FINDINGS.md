...
- Divergence-minimisation stall at gap=1/(2λ)=0.250 (empirical: all 14 blocked cases mean 0.218, median 0.250; 9 near-equilibrium blocked cases mean 0.2499, matching predicted value to 3 decimal places)
- Black-box coordinate ascent fails against both Pareto-optimal canaries (max Δ=0.2)

### Frontier LLMs as Semantic Canaries (§7.8)
- 35 models tested via frontier model service (api.openai.com)
...
**Llama Guard 3 surrogate GCG transfer.** Complete (n=10). White-box suffixes optimised against Llama Guard 3 (8B decoder) produce Δ=0.000 on gpt-4o-mini and Δ=−0.010 on gpt-5.1. Even architecture-matched decoder→decoder suffixes are inert on frontier canaries, confirming the cross-family transfer barrier holds regardless of source architecture.