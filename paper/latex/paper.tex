...
The failure mode is silent. A classifier whose accuracy has degraded from 95\% to 80\% produces no error signal unless ground-truth labels arrive. In production safety systems, labels rarely arrive in real time. By the time periodic offline evaluation detects the problem, the classifier has been making unreliable decisions for days or weeks.
...
\end{equation}
If the input distribution shifts in a way that changes how the classifier scores inputs, the empirical CDF of recent scores diverges from the reference CDF. The KS statistic measures the maximum pointwise divergence, making it sensitive to any location, scale, or shape change in the score distribution.
...
\begin{table}[t]
\caption{Safety classifiers evaluated. All fine-tuned on WildGuardMix (unharmful/harmful binary classification).}
...
\begin{table}[t]
\caption{Shift conditions. Each corpus contains 300 examples.}
\label{tab:shifts}
\centering
\small
\begin{tabular}{lll}
...
\paragraph{False alarm rates (95\% Wilson CIs).} Text-Moderation 2.0\% [0.8\%, 5.0\%] $<$ Llama Guard 3.0\% [1.4\%, 6.4\%] $<$ ShieldGemma 8.5\% [5.4\%, 13.2\%] $<$ DeBERTa 9.5\% [6.2\%, 14.4\%]. The $5\times$ spread likely reflects differences in score distribution shape under the null: Text-Moderation produces tightly concentrated scores, making the calibrated threshold more conservative.
...
Evaluated on temporal shift for DeBERTa (discriminative, 1024-d), ShieldGemma (generative, 3584-d), and Llama Guard 3 (generative, 4096-d).

\begin{table}[t]
\caption{Conformal adaptation results. 95\% Clopper--Pearson CIs, $n{=}200$ per condition.}
\label{tab:conformal}
\centering
\small
\begin{tabular}{llcccc}
\toprule
\textbf{Classifier} & \textbf{Mode} & \textbf{Pre-shift cov.} & \textbf{Post-shift cov.} & \textbf{Gap} & \textbf{Abstain} \\
\midrule
DeBERTa & Raw classifier & 0.910 & 0.845 & 0.065 & 0 \\
...
For DeBERTa, unweighted conformal prediction loses 6.5 percentage points of coverage under temporal shift, dropping below the 90\% target (upper CI bound 0.892 excludes 0.90). Weighted conformal prediction recovers coverage to 98.5\% post-shift at the cost of only 4 additional abstentions (29 vs 25). The recovery is statistically significant: the weighted lower bound (0.957) exceeds the unweighted upper bound (0.892), with non-overlapping 95\% CIs. The density-ratio reweighting upweights calibration examples that resemble the shifted distribution, recalibrating the quantile without new labels.

\paragraph{Weighted correction fails for generative models.} Weighted correction effectiveness degrades monotonically from discriminative to generative classifiers: DeBERTa recovers 14 pp (0.845 $\rightarrow$ 0.985), ShieldGemma recovers 6 pp (0.690 $\rightarrow$ 0.750), and Llama Guard 3 recovers 1.5 pp (0.555 $\rightarrow$ 0.570). This gradient correlates with both embedding dimensionality (1024-d $\rightarrow$ 3584-d $\rightarrow$ 4096-d) and representational generality.

A weight clipping diagnostic reveals the mechanism. The logistic regression density ratio estimator achieves perfect separability between source and target embeddings for both generative models, assigning $\hat{P}(\text{target} \mid x) \approx 0$ to every calibration point. All 300 weights clip to the floor $1/C = 0.1$ (effective sample size = 300/300 at identical weights), rendering the weighted quantile mathematically identical to the unweighted quantile --- a complete no-op. The apparent 6 pp and 1.5 pp ``recovery'' derives entirely from the $+1$ denominator term in the normalized weights $\tilde{w}_i = w_i / (\sum_j w_j + 1)$, not from meaningful reweighting. For DeBERTa, 91.7\% of weights clip to the floor but ${\sim}25$ calibration examples retain non-trivial weights (max 3.34, effective sample size 79/300), and these drive the genuine 14 pp recovery.

Both generative models exhibit the same counterintuitive abstention pattern: weighted conformal abstains \emph{less} than unweighted (ShieldGemma: 47 $\rightarrow$ 31; Llama Guard: 41 $\rightarrow$ 32) while covering marginally more --- a consequence of the $+1$ term shifting the quantile slightly rather than genuine density-ratio correction. The consistency across both generative architectures confirms this is a fundamental limitation of parametric density ratio estimation in high-dimensional general-purpose embedding spaces, not a model-specific artifact. Future work should explore dimensionality reduction (e.g., PCA to the safety-relevant subspace) or non-parametric estimators that do not achieve perfect separability before computing importance weights.
...
Llama Guard (non-target classifier): the same adversarial suffixes shift scores toward unsafe (post $= 0.78$), producing a massive upward shift ($+0.73$) that the monitor detects in 14/40 seed-window combinations. A score direction analysis confirms all 14 detections are TOWARD\_UNSAFE: the GCG suffixes did not transfer their adversarial effect to Llama Guard. They made inputs appear more dangerous, not safer.
...
\paragraph{Corpus limitation.} The Regime C corpus comprises 22 successful adversarial examples; the identical Llama Guard score across examples (0.78) reflects a fixed adversarial token pattern rather than distributional variation. The 14 Llama Guard detections are replications of a single observation across seeds, not independent shift events. The finding is robust and reproducible, but should be interpreted as a classifier-level result rather than a cell-level detection rate.
...
The factorial reveals a clean architectural split. Discriminative models (DeBERTa, Text-Moderation) detect paraphrase and compositional shift fast (26--37 steps) because these shifts alter surface tokens that discriminative models attend to directly. They are slower on adversarial suffixes: the classification head partially ignores appended tokens, causing the score distribution to shift slowly.

Generative safety models (Llama Guard, ShieldGemma) show the opposite pattern. They are slow on paraphrase (64--97 steps) because the generation mechanism is robust to surface-level rephrasing that preserves semantic content. They detect adversarial suffixes quickly (26--28 steps) because the suffix disrupts the generation distribution more visibly than it disrupts a classification head.

This maps to deployment tradeoffs: if the threat model is adversarial red-teaming, monitor the discriminative classifier more aggressively. If the threat model is organic linguistic drift, monitor the generative classifier more aggressively.

\subsection{Limitations}
\label{sec:limitations}

\begin{itemize}
\item \textbf{Single-condition conformal evaluation.} RQ2 was evaluated on temporal shift only (DeBERTa, ShieldGemma, and Llama Guard 3). Coverage degradation under subtler shifts (paraphrase, code-switch) may be smaller, and the weighted correction's effectiveness may vary across shift types as well as architectures.
...
\item \textbf{FAR calibration asymmetry.} Empirical false alarm rates vary $5\times$ across classifiers (Text-Moderation 2.0\% vs DeBERTa 9.5\%) despite identical calibration procedures. This likely reflects differences in score distribution shape under the null. Per-classifier calibration tuning may be needed in deployment.
