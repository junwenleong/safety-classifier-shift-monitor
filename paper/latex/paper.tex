...
\item \textbf{Joint-optimisation sample size.} Joint GCG results (\S\ref{sec:joint-gcg}) use $n = 10$ prompts at a single $\lambda$. Larger-scale sweeps may reveal additional failure modes.
\item \textbf{Phase-transition derivation.} The stall condition (Appendix~\ref{app:derivation}) uses a continuous relaxation; GCG operates on discrete tokens. The match is empirical (within 95\% CI), not a formal convergence proof.
