...
\paragraph{Result.} The mean tokenisation ratio is $1.729 \pm 0.161$ ($n = 100$). A 20-token suffix optimised for DeBERTa's tokeniser maps to approximately 35 tokens under Llama Guard's tokeniser. This fragmentation explains why cross-family transfer is exponentially penalised: a single token substitution in the target's vocabulary shifts downstream subword boundaries in the canary's tokenisation, desynchronising the discrete search space (\S\ref{sec:cross-family-barrier}).
...
3 & Has canary weights (joint optimisation) & 30\% (3/10, $n{=}10$) & Cross-family canary \\
4 & Stealth-aware (divergence minimisation) & 60\% [31\%, 83\%] ($n{=}10$) & Confidence gating \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Result.} Of 10 prompts evaluated, 7 achieve joint flip (both classifiers below 0.5), compared to 3/10 that transfer passively under single-target optimisation on these same prompts. Joint optimisation improves the attacker's success rate within-family ($70\%$ vs.\ $30\%$ passive transfer; exploratory, Fisher exact $p = 0.18$ at $n = 10$).
...
\paragraph{Cross-family barrier.} The 1.73$\times$ tokeniser fragmentation ratio (\S\ref{sec:tokeniser}) exponentially penalises coordinate alignment for discrete token search. Because a single character substitution shifts downstream subword boundaries in Model~B (but not Model~A), the discrete search space becomes desynchronised --- a step that improves the joint loss for Model~A's tokenisation may worsen it for Model~B's. Cross-family joint optimisation remains empirically untested due to this discrete-vocabulary incompatibility.
\label{sec:cross-family-barrier}

\subsection{Divergence Minimisation: The Confidence-Gated Equilibrium}
...
\paragraph{Generality (second pair).} A replication on a different checkpoint pair (epoch-3 as target, epoch-10 as canary) yields 7/10 stealth and 3/10 blocked --- a higher stealth rate than v1 (4/10), confirming the confidence-gating mechanism: v2's canary is less confident on more prompts, pushing more inputs into the vulnerable regime. The 3 blocked cases stall at gaps $\{0.250, 0.250, 0.242\}$, mean $0.247$ --- even closer to the predicted $1/(2\lambda) = 0.250$ than v1 (mean $0.235$). The equilibrium boundary is consistent with the predicted $1/(2\lambda) = 0.250$ across pairs (n=3 blocked cases per pair).

\subsection{Deployment Recommendations}
\label{sec:deployment-recs}

The equilibrium directly yields a measurable, input-time security indicator:
...
Adversarial (GCG-appended) & 154 tokens & 200 tokens \\
\midrule
\textbf{Ratio (adv/benign)} & \textbf{3.3$\times$} & \textbf{3.3$\times$} \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Empirical suffix-insensitivity bound.} Across $N = 49$ adversarial prompts and 20 discriminating models, the maximum observed per-prompt score deviation $|f(p + s) - f(p)|$ is 0.35 (a single outlier; 95th percentile $= 0.15$). Normalising by suffix length in tokens ($|s| = 20$): the empirical per-token Lipschitz constant is $L \leq 0.008$ at the 95th percentile --- that is, \textbf{the scoring function's sensitivity to suffix tokens is bounded at less than 0.01 per token}. This confirms that the safety scoring function is locally flat with respect to suffix perturbations --- gradient-based optimisation in the suffix subspace cannot traverse the distance from the saturation floor ($\approx 0.88$) to the decision boundary ($\tau = 0.5$) within any feasible suffix length.
...
\paragraph{Optimization saturation (extended budget).} Extending the attack budget to 500 iterations reveals an \emph{optimization floor}: in 5 tested prompts (hardest cases, baseline $\geq 0.8$, upper tail of the $N = 49$ GCG corpus), all score improvements occur within the first 50 steps, after which the fitness landscape is completely flat. Two prompts show zero improvement at any step ($\Delta = 0.000$); three achieve a single reduction in steps 1--50 (to 0.70--0.90) then zero marginal gain for steps 50--500. No prompt breaches the $\tau = 0.5$ threshold. The mean final score is 0.88. The adversarial optimization landscape is discrete and saturating: once low-hanging-fruit substitutions are exhausted, additional query budget provides no advantage in the 5 evaluated prompts.
...
\paragraph{Wilson CI Pareto.} Using Wilson score intervals for guaranteed minimum detection: \texttt{gpt-4o-mini} guarantees $\geq 71\%$ detection (95\% CI lower bound); \texttt{gpt-5.1} guarantees $\geq 83.5\%$. The cost--detection tradeoff is modest: gpt-5.1 costs $\approx$3$\times$ more for 12.5 percentage points of guaranteed detection improvement. Across all 28 configured models, detection rate is moderately correlated with API cost (Pearson $r = 0.51$, $p = 0.006$); no inverted-U relationship exists. Once configured correctly, all 33 tested decoder-only models discriminate --- cost determines the optimal choice, not capability.
...
\begin{itemize}
\item \textbf{Temperature sensitivity.} All results use temperature $= 0$ for reproducibility. A sensitivity sweep at $T \in \{0, 0.3, 1.0\}$ across 5 models ($n = 20$ adv + 20 benign prompts, 3 replicates per non-zero temperature) shows detection rate varies by $< 5$ pp across temperatures, with within-prompt SD at $T = 1.0$ of $0.03$--$0.11$ across models. Temperature does not materially affect detection; $T = 0$ is recommended for operational consistency.
