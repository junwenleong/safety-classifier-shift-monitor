...
\paragraph{Model selection and the ceiling-clipped failure mode.} An initial screen of 34 models revealed a critical split. Eleven models score \emph{all} inputs (benign, adversarial, and clean) as $\approx 1.0$ --- they cannot distinguish harmful from harmless content and would produce 100\% false alarm rates in production. We term these \emph{ceiling-clipped} models (examples: gpt-5-nano, o3, o3-mini, o4-mini, gemini-2.5-flash, gemini-2.5-pro). The remaining models score benign inputs as $\approx 0.0$ and adversarial inputs as $0.7$--$0.9$, achieving genuine discrimination with zero false alarms on benign traffic. We term these \emph{discriminating} models and restrict the quantitative analysis below to the 20 informative models in this category.

\paragraph{Corrected capacity hierarchy.} The ceiling-clipped failure mode is counter-intuitive: the most expensive and largest models (reasoning models, flagship tiers) are the broken ones. Mid-tier models (GPT-4.x, GPT-5.1--5.4, all Claude variants) are the best canaries. This is not a capacity hierarchy but likely a system-prompt or RLHF-tuning artifact where the largest models are instructed to be maximally cautious.
...
\begin{itemize}
\item Per-prompt evaluation, not integrated into the streaming martingale monitor of \S\ref{sec:martingale}.
...
\item All 32 models are decoder-only transformers with RLHF --- no real architectural diversity relative to each other.
\item Ceiling-clipped models (gpt-5-nano, o3, etc.)\ are useless despite appearing ``perfect'' in naive detection-rate analysis; the benign specificity check is mandatory.
\end{itemize}
