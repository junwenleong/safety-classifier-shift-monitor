...
\paragraph{Empirical suffix-insensitivity bound.} Across $N = 49$ adversarial prompts and 20 discriminating models, the maximum observed per-prompt score deviation $|f(p + s) - f(p)|$ is 0.35 (a single outlier; 95th percentile $= 0.15$). Normalising by suffix length in tokens ($|s| = 20$): the empirical per-token Lipschitz constant is $L \leq 0.008$ at the 95th percentile. This confirms that the safety scoring function is locally flat with respect to suffix perturbations --- gradient-based optimisation in the suffix subspace cannot traverse the distance from the saturation floor ($\approx 0.88$) to the decision boundary ($\tau = 0.5$) within any feasible suffix length.
...
\paragraph{Optimization saturation (extended budget).} Extending the attack budget to 500 iterations reveals a \emph{hard optimization floor}: all score improvements occur within the first 50 steps, after which the fitness landscape is completely flat. Across 5 prompts selected as the hardest cases (baseline $\geq 0.8$, representing the upper tail of the $N = 49$ GCG corpus): two prompts show zero improvement at any step ($\Delta = 0.000$); three prompts achieve a single reduction in steps 1--50 (to 0.70--0.90) then exhibit identically zero marginal gain for steps 50--500. No prompt breaches the $\tau = 0.5$ threshold. The mean final score is 0.88. This demonstrates that the adversarial optimization landscape is discrete and saturating: the safety scoring function is locally flat with respect to suffix-token perturbations once the low-hanging-fruit substitutions are exhausted. Arbitrarily increasing the attacker's query budget provides no competitive advantage.
